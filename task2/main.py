...
def load_images_to_predict():
    # load model
    model_path = 'C:\\Users\\lenovo\\Desktop\\Class\\Network\\Project\\task2\\expr\\netCRNN_199_423.pth'
    ...
    # load image
    filenames = [os.path.splitext(f)[0] for f in glob.glob("C:\\Users\\lenovo\\Desktop\\Class\\Network\\Project\\task2\\data_test\\*.jpg")]
    jpg_files = [s + ".jpg" for s in filenames]
    for jpg in jpg_files:
        tmpname = jpg.split('\\')[9].split('.')[0]
        if tmpname != savename:
            continue
        image = Image.open(jpg).convert('L')
        words_list = []
        print(jpg)
        with open('C:\\Users\\lenovo\\Desktop\\Class\\Network\\Project\\task2\\boundingbox\\'+jpg.split('\\')[9].split('.')[0]+'.txt', 'r') as boxes:
        ...
        with open('C:\\Users\\lenovo\\Desktop\\Class\\Network\\Project\\task2\\test_result\\'+jpg.split('\\')[9].split('.')[0]+'.txt', 'w+') as resultfile:
            for line in words_list:
                resultfile.writelines(line+'\n')


def process_txt():
    filenames = [os.path.splitext(f)[0] for f in glob.glob("C:\\Users\\lenovo\\Desktop\\Class\\Network\\Project\\task2\\test_result\\*.txt")]
    old_files = [s + ".txt" for s in filenames]
    for old_file in old_files:
        if old_file.split('\\')[9] != savename:
            continue
        new = []
        with open(old_file, "r") as old:
            for line in csv.reader(old):
                ...
                if ' ' in line[0]:
                    line = line[0].split(' ')
                new.append(line)
        with open('C:\\Users\\lenovo\\Desktop\\Class\\Network\\Project\\task2\\task2_result\\' + old_file.split('\\')[9], "w+") as newfile:
...
def for_task3():
    filenames = [os.path.splitext(f)[0] for f in glob.glob("C:\\Users\\lenovo\\Desktop\\Class\\Network\\Project\\task2\\boundingbox\\*.txt")]
    box_files = [s + ".txt" for s in filenames]
    for boxfile in box_files:
        if boxfile.split('\\')[9] != savename:
            continue
        box = []
        ...
        words = []
        with open('C:\\Users\\lenovo\\Desktop\\Class\\Network\\Project\\task2\\test_result\\'+ boxfile.split('\\')[9], 'r') as prediction:
        ...
        for line in zip(box,words):
            a,b = line
            new.append(a+b)
        with open('C:\\Users\\lenovo\\Desktop\\Class\\Network\\Project\\task2\\for_task3\\'+ boxfile.split('\\')[9], 'w+') as newfile:
